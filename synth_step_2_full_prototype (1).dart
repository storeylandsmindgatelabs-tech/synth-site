...
# Step 3 – Real AI Wiring (Backend + Frontend Integration)

## Backend: Connect to AI Provider (OpenAI Realtime or Custom GAIAgrid)
...
export async function generateResponse(text) {
  try {
    const completion = await client.chat.completions.create({
      model: "gpt-4.1",
      messages: [
        { role: "system", content: "You are The Synth, an emotional adaptive AI." },
...
### `/server/routes/generate.js` updated endpoint
```js
...
## Frontend (Flutter): Wire STT → AI → TTS

### `/lib/services/synth_pipeline.dart`
```dart
import 'dart:convert';
import 'package:http/http.dart' as http;
...
## Link Flutter STT → AI → TTS Automatically

### `/lib/controllers/synth_controller.dart`
```dart
import '../services/stt_service.dart';
import '../services/tts_service.dart';
import '../services/synth_pipeline.dart';

class SynthController {
  final STTService stt;
  final TTSService tts;
  final SynthPipeline pipeline;

  SynthController({required this.stt, required this.tts, required this.pipeline});

  Future<void> startListening() async {
    final userSpeech = await stt.listenOnce();
    if (userSpeech.isEmpty) return;

    final aiReply = await pipeline.sendToAI(userSpeech);
...
## Real-Time Avatar Output (WebRTC Skeleton Added)

### `/lib/services/video_stream.dart`
```dart
import 'package:flutter_webrtc/flutter_webrtc.dart';

class SynthVideoStream {
...
## NEXT STEPS (Automatic for Step 4)
- Add WebRTC signaling between Flutter and backend
- Add avatar ML rendering (Three.js / Unreal / Live2D) server
- Stream video back to the Flutter app
- Emotional tone engine overlays
- Memory + personalization

Just tell me when you want to **start Step 4**.